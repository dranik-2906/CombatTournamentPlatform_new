...
class FightService:
    ...
    def complete_fight(fight, winner=None, method='', notes='', scores=None):
        ...
        # Продвигаем победителя дальше (для single-elimination / mixed)
        ...
        # === АВТО-СОЗДАНИЕ ПЛЕЙ-ОФФ ПОСЛЕ ROUND-ROBIN ===
        if fight.bracket:
            FightService._maybe_create_playoff(fight.bracket)

        return True, "Бой завершён"

    @staticmethod
    def _maybe_create_playoff(bracket):
        """Если все бои группы завершены и плей-офф ещё не создан — создать"""
        # Проверяем, есть ли уже плей-офф (бои с round_number > 1)
        has_playoff = bracket.fights.filter(round_number__gt=1).exists()
        if has_playoff:
            return

        # Все бои завершены (или отменены)?
        unfinished = bracket.fights.exclude(status='completed').exclude(status='cancelled')
        if unfinished.exists():
            return

        # Если тип round_robin — автоматически создаём плей-офф
        if bracket.bracket_type == 'round_robin':
            success, msg = FightService.create_playoff_from_round_robin(bracket)
            if success:
                logger.info(f"Auto-playoff created for bracket {bracket.id}: {msg}")
    ...
    def create_playoff_from_round_robin(bracket, advancing_count=None):
        ...
        # Автоопределение количества проходящих
        ...
        # Меняем тип сетки на mixed (группа + плей-офф)
        bracket.bracket_type = 'mixed'
        bracket.save(update_fields=['bracket_type'])
    ...
    def _round_robin_standings(bracket):
        ...
        for f in fights:
            ...
            if f.winner:
                stats[f.winner.id]['wins'] += 1
            elif f.is_draw:
                if f.fighter1_id in stats:
                    stats[f.fighter1_id]['draws'] += 1
                if f.fighter2_id in stats:
                    stats[f.fighter2_id]['draws'] += 1
            if f.fighter1_id in stats:
                stats[f.fighter1_id]['points_for'] += f.score_fighter1
                stats[f.fighter1_id]['points_against'] += f.score_fighter2
            if f.fighter2_id in stats:
                stats[f.fighter2_id]['points_for'] += f.score_fighter2
                stats[f.fighter2_id]['points_against'] += f.score_fighter1

        items = sorted(
            stats.values(),
            key=lambda x: (x['wins'], x['draws'], x['points_for'] - x['points_against'], x['points_for']),
            reverse=True
        )
        result = []
        for i, item in enumerate(items, 1):
            result.append({
                ...
                'method': f"Побед: {item['wins']}, Очков: {item['points_for']}-{item['points_against']}"
            })
        return result
